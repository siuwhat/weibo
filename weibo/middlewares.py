...
#设置代理ip的服务器
    ...
    def process_request(self, request, spider):
        if not self.PROXY_IP_LIST:
            self.GetIp()
        random_ip = random.choice(self.PROXY_IP_LIST)
        ip='http://'+random_ip+':'+'8888'
        print(ip)
    ...
    def process_exception(self,request,exception,spider):
        print('转换ip 中')
        self.changeip()
        request.meta['proxy'] = 'http://'+ random.choice(self.PROXY_IP_LIST)+':8888'
